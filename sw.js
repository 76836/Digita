...
<button onclick="location.href='/Digita/settings'">Digita settings</button>
</div>
<div id="term">
<div id="output"></div>
...
async function skipCache(){
    try{
        const cache=await caches.open('DigitaPWACache');
...
async function handleCommand(cmd){
    ...
    switch(cmd.toLowerCase()){
        ...
        case 'info':
            ...
            break;
        case 'version':
            log(\`Recovery Page v5 for DigitaPWA\nService Worker Recovery System\`);
            break;
        default:
            try{
                log(await eval(cmd));
...
if (url.origin === self.location.origin && url.pathname.startsWith('/Akari/Digita')) {
  ...
  // Apply custom headers for Digita to enable multi threading on WASM 
      if (url.origin === self.location.origin && (url.pathname.startsWith('/Digita'))) {
